...
fn main() {
    ...
    thread::scope(|s| {
        // Writer thread: Updates the store repeatedly using JSON Patch
        let s_writer = &store;
        s.spawn(|| {
            for i in 1..=5 {
                thread::sleep(Duration::from_millis(100));
                println!("Writer: Updating to generation {} using JSON Patch", i);

                // Mirroring the Merge Patch in shared_store.rs:
                // {"metadata": { "version": i + 1 }, "counters": { "a": i * 10 }}
                s_writer
                    .update(json!([
                        {"op": "replace", "path": "/metadata/version", "value": i + 1},
                        {"op": "replace", "path": "/counters/a", "value": i * 10}
                    ]))
